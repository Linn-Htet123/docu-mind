...
export class PipelineService {
    ...
    private needsRewriting(message: string): boolean {
        ...
        const hasReferences = /\b(he|him|his|she|her|hers|it|its|they|them|their|theirs|that|this|those|these|one|ones)\b/i.test(message);
        const isContinuation = /^(and|but|so|or|also|plus|besides|additionally|furthermore|moreover|what about|how about|tell me (more|about)|i mean|more on|explain that|regarding|speaking of|as for)\b/i.test(trimmed);
        const isVagueRequest = /\b(more|elaborate|explain|clarify|expand|continue|go on|details?|deeper|further|specific)\b/i.test(trimmed) && message.split(/\s+/).length < 8;
        const isAcknowledgment = /^(yes|no|yeah|yep|nope|yup|nah|ok|okay|sure|right|exactly|correct|true|false|agreed|got it|i see|makes sense)\b/i.test(trimmed);
        const hasComparatives = /\b(other|another|different|same|similar|like that|such as)\b/i.test(trimmed) && message.split(/\s+/).length < 7;
        const isIncompleteQuestion = /^(why|when|where|how|what|which|who)(\s+(about|so|then|else|now))?[\?\s]*$/i.test(trimmed);
        const isFormatDirective = /^(in|as|with|make it|format|use|show me|give me|list|bullet|short|brief|summary|table)/i.test(trimmed) && message.split(/\s+/).length < 6;
        const isShort = message.split(/\s+/).length < 5;

        return hasReferences || isContinuation || isVagueRequest || isAcknowledgment ||
            hasComparatives || isIncompleteQuestion || isFormatDirective || isShort;
    }
